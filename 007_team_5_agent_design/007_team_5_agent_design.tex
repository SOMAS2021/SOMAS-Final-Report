...
\begin{align*}
    ...
    \texttt{unboundedFavour} &= \begin{cases}
        \texttt{originalFavour} + 1, \hfill \texttt{judgement} > 0.075\\
        \texttt{originalFavour} + \max{\left(\frac{\texttt{judgement}}{2}, -3\right)}, \hfill\texttt{judgement} < -2
    \end{cases}\\
    \texttt{favour} &= \begin{cases}
        0, \hfill\texttt{unboundedFavour} < 0\\
        \texttt{unboundedFavour}, \quad\hfill 0 <= \texttt{unboundedFavour} <= 10\\
        10, \hfill\texttt{unboundedFavour} > 10
    \end{cases}\\
\end{align*}
