...
All agents \textit{i} $\in$ \textit{A} are implemented as a data structure, such that all agents contain sufficient parameterisation to participate in the various communication methods \textit{c} $\in$ \textit{C}, resulting in a set of interactions defined by I = $< \mathit{A, C} >$. Each agent inherits from the \texttt{baseAgent} structure and also contains the fields contained in Table \ref{tab:agentStruct}. We note only the most relevant fields for quantifying the agent have been included.
...
This weighted sum represents the prediction of the agent's next social motive, named \texttt{nextBeha- viourPrediction} in \Cref{fig:socialMotives}. We therefore calculate:
...
If the change as defined in \Cref{willingness_to_change} results in a \texttt{newBehaviour} outside this range, then this is automatically capped off at the edges. Naturally, this also holds for the spectrum edges $[0,10]$.
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item  If the agent accepts the treaty: +2
        \item   If the agent rejects the treaty: $-1$
    \end{itemize}
\end{enumerate}
...
Conversely, in the case of a negative exchange, the scaling of the trust update of another agent will \textit{decrease} as follows:
\begin{itemize}
...
\begin{enumerate}
    ...
    \item Calculate the expected short- and long-term utility according to \Cref{eq:expectedUtility} using the memory of food received in the past. The utility function used depends on its current social motive.
    ...
    \item Choose to focus on the long-term or short-term benefit according to \Cref{eq:shortlongweight}
